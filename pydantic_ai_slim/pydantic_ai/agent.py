...
class AgentRunResult(Generic[OutputDataT]):
    ...
    def new_messages(self, *, output_tool_return_content: str | None = None) -> list[_messages.ModelMessage]:
        """Return new messages associated with this run. Only copy/adjust if needed."""
        # Inline coalesce_deprecated_return_content for speed
        content = output_tool_return_content
        if content is None:
            ...
            content = result_tool_return_content

        if content is not None:
            # Avoid full call to all_messages, just copy and slice once.
            messages = self._set_output_tool_return(content)
            return messages[self._new_message_index :]
        else:
            # Slicing avoids a copy for the full list if not needed
