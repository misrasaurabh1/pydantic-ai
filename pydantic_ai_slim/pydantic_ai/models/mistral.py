...
class MistralStreamedResponse(StreamedResponse):
    ...
    def _validate_required_json_schema(json_dict: dict[str, Any], json_schema: dict[str, Any]) -> bool:
        ...
        if not required_params:
            return True  # No required fields, consider as valid

        properties = json_schema.get('properties')
        if not properties:
            # No property constraints; treat as simple key presence check.
            for param in required_params:
                if param not in json_dict:
                    return False
            return True

        # Avoid attribute lookups and function calls in loop.
        ...
        for param in required_params:
            ...
            if not param_schema:
                continue  # If no schema for property, skip type checking.

            param_type = param_schema.get('type')
            param_value = json_dict[param]

            if param_type == 'array':
                param_items_schema = param_schema.get('items')
                if not isinstance(param_value, list):
                    return False
                item_type = param_items_schema and param_items_schema.get('type')
                if item_type:
                    item_pytype = vjtm[item_type]
                    for item in param_value:
                        if not isinstance(item, item_pytype):
                            return False

            elif param_type:
                param_pytype = vjtm[param_type]
                if not isinstance(param_value, param_pytype):
                    return False

            # For nested object with property constraints
