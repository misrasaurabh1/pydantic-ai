...
class AzureProvider(Provider[AsyncOpenAI]):
    ...
    def model_profile(self, model_name: str) -> ModelProfile | None:
        ...
        # Fast-path checking, no need to import or call absent profile functions
        # Prefix tuple: (prefix, result from inline fast-null profile function, strip flag, use meta_model_profile)
        prefix_profiles = (
            ('llama', None, False, True),
            ('meta-', None, True, True),
            ('deepseek', None, False, False),
            ('mistralai-', None, True, False),
            ('mistral', None, False, False),
            ('cohere-', None, True, False),
            ('grok', None, False, False),
        )

        for prefix, _, strip, is_meta in prefix_profiles:
            if model_name_lower.startswith(prefix):
                cut_name = model_name[len(prefix) :] if strip else model_name
                profile = meta_model_profile(cut_name) if is_meta else None
                # Maintain OpenAIModelProfile base by default (see below).
                return OpenAIModelProfile(json_schema_transformer=OpenAIJsonSchemaTransformer).update(profile)

        # OpenAI models are unprefixed
        return openai_model_profile(model_name_lower)
